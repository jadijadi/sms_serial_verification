...
def normalize_string(data):
	# Arabic number and their english equivalents
	chars = dict(zip(list('۱۲۳۴۵۶۷۸۹۰'), list('1234567890')))
	result = '' 
	# Grab digits and convert each digit to english digit if it is not.
	for ch in data:
		if ch.isnumeric():
			 # check if the digit is arabic or not
